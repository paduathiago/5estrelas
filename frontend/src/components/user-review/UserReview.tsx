...
function UserReview({
  ...props
}: Review & { establishment: any; reviewComment?: string }) {
  ...
  const [commentDraft, setCommentDraft] = useState<string>(
    props.establishmentComment || ""
  );

  const establishmentComment = props.reviewComment
    ? JSON.parse(props.reviewComment).comment
    : "";
  ...
  const image = props.userImage
    ? JSON.parse(props.userImage).base64
    : undefined;
      ...
      <CardFooter className="flex flex-col gap-4 items-start">
        <div className="flex justify-between gap-2 w-full">
          <div className="flex gap-2 items-center">
            ...
            <p className="text-sm">
              {formatDateToString(new Date(props.timestamp))}
            </p>
          </div>
          <div className="flex gap-2">
            {props.establishment?.isFromUser && (
              <DialogCloseButton
                onSendAnswer={(com: string) => {
                  setComment(com);
                }}
                reviewId={props.id}
              ></DialogCloseButton>
            ...
            <Button
              ...
              className="gap-1"
              onClick={() => updateUserFeedback("LIKE")}
            >
              <ThumbsUp />
              {feedbackNumber["LIKE"]}
            </Button>
            ...
            <Button
              ...
              className="gap-1"
              onClick={() => updateUserFeedback("DISLIKE")}
            >
              <ThumbsDown />
